...
import io.exzocoin.wallet.core.IChartTypeStorage
...
import io.exzocoin.wallet.entities.TransactionDataSortingType
import io.exzocoin.wallet.modules.balance.BalanceSortType
import io.exzocoin.wallet.modules.market.MarketModule
...
import io.exzocoin.core.IThirdKeyboard
...
class LocalStorageManager(private val preferences: SharedPreferences)
    ...
    private val AUTH_USER = "auth_user";
    ...
    override var currentTheme: ThemeType
        get() = preferences.getString(CURRENT_THEME, null)?.let { ThemeType.valueOf(it) } ?: ThemeType.Light
        set(themeType) {
    ...
    override var authUser: UserInfo?
        get() {
            var userStr = preferences.getString(AUTH_USER, "")
            if (userStr == "") {
                return null
            }
            return UserInfo.parseFromJson(userStr)
        }
        set(value: UserInfo?) {
            var userStr = value?.toJson().toString();
            preferences.edit().putString(AUTH_USER, userStr).apply()
        }
}
